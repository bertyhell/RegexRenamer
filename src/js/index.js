...
document.addEventListener('deviceready', function () {
    ...
    $("#btnPreview").click(function () {
        ...
        for (var i = 1; i < files.length; i++) {
            var folderName = $(files[i]).attr("name");
    ...
    $("#btnRename").click(function () {
        ...
        var files = $("li");
        for (var i = 1; i < files.length; i++) {
            var currentName = $(files[i]).attr("name");
            var newName = $(files[i]).find("span + span").html();
    ...
    $("li[class='folder']").click(function (element) {
        if ($(element.currentTarget).html() == "..") {
            ...
            if ($(element.currentTarget).hasClass("folder")) {
                var subFolder = $(element.currentTarget).find("span:first-child").html();
